...
function artifacts = artifactsWAV(directory,channelFile,prefix)
...
dispstat('','init');
dispstat(sprintf('Summarizing WAV artifacts by subject. Please wait...'),'keepthis');
...
for fileCounter = 1:fileCount
    ...
    artifactData(fileCounter,1) = squeeze(mean(cell2mat(subjectData.WAV.nAccepted)));
    artifactData(fileCounter,2) = squeeze(mean(cell2mat(subjectData.WAV.nRejected)));
end
