...
function raw = summarizeWAV(directory,channelFile,prefix)
...
dispstat('','init');
dispstat(sprintf('Summarizing WAV by subject. Please wait...'),'keepthis');
...
for fileCounter = 1:fileCount
    ...
    conditionCount = size(subjectData.WAV.conditions,2);
    ...
    for conditionCounter = 1:conditionCount
        ...
        for channelCounter = 1:channelCount
            ...
            tempData(channelCounter,:,:,conditionCounter,fileCounter) =...
                subjectData.WAV.data{conditionCounter}(channelIndex,:,:);
        end
    end
end
